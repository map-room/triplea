 ...
 *
 * <p>Format: {@code [AI-TRACE] matchID=<id> side=sidecar nation=X phase=Y key=value ...} Arrays
 * use {@code [a,b,c]} notation; unit counts use {@code type×N} notation; territory names
 * containing spaces are double-quoted.
 *
 * <p>The {@code matchID} tag is sourced from {@link #setMatchId(String)}, which the request
 * boundary in {@code DecisionHandler} sets before dispatching an executor and clears in a
 * finally block. The system uses a per-thread context (functionally an MDC slot for {@code
 * System.Logger}-based code) — every executor call site already runs on the HTTP request
 * thread, so a {@link ThreadLocal} carries the matchID through the executor stack without
 * having to thread it through every method signature.
 *
 * <p>If no matchID is set when an emit happens (e.g. tests, or a background path that never
 * went through {@code DecisionHandler}), the line is tagged {@code matchID=unknown} so the
 * Loki query {@code {matchID=~"unknown|..."} } still surfaces those lines for triage.
 */
public final class AiTraceLogger {
  ...
  /**
   * Bind the matchID for the current thread. Call at the request boundary
   * (e.g. start of {@code DecisionHandler.handle}). Pair with {@link #clearMatchId()} in a
   * finally block so a thread-pool worker doesn't leak a stale matchID into the next request.
   */
  ...
  /**
   * Read the per-thread matchID, falling back to {@code "unknown"} when no boundary set one.
   * Package-private so executor tests can assert without going through the HTTP layer.
   */
